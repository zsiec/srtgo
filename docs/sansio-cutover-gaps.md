...
| ~~No file-mode congestion control (FileCC)~~ | **DONE** | `Congestion` now flows through to `establish` (`Config`/`DialConfig`/`ListenerConfig` → `establishParams`); `Congestion=="file"` selects `congestion.NewFileCC` (window-based AIMD slow start) instead of `NewLiveCC`. The core's `window()` already honors `CongestionWindow`, so FileCC's adaptive cwnd gates the send window. Test `TestSimFileModeCongestion` (initial cwnd 16 proves FileCC vs LiveCC; lossy transfer completes in order; **cwnd now grows under virtual time** — see the injected-clock row below). |
| ~~FileCC rate control on the wall clock (non-deterministic)~~ | **DONE** | `Controller.OnACKAt(now, …)` added: the Sans-I/O core feeds FileCC its loop `now` (handleACK), so the 10ms rate-control gate is a function of the injected clock, not `time.Now()`. `lastRCTime` is now a `clock.Timestamp` seeded lazily on the first ACK (matching whichever epoch the caller uses), so file-mode cwnd advances deterministically in sim. Legacy + the congestion package's own tests keep `OnACK` (wall clock); the two collapse after cutover. `TestSimFileModeCongestion` now asserts cwnd > 16 after the transfer. |
...
| ~~Wire flags hardcoded to defaults~~ | **DONE** | Both sides now advertise computed flags via the shared `srtFeatureFlags` (TSBPD in live, Crypt when encrypted, TLPKTDROP when set, periodic NAK unless file/NAKREPORT=false, byte-stream for stream mode): the caller's CONCLUSION (`dialState.srtFlags`) and the listener's CONCLUSION response (`Listener.srtFlags`). Both sides parse the peer's advertised `FlagPeriodicNAK` into `peerNakReport` (`peerNakReportFromHS`), surfaced as `Stats.PeerNakReport`. FASTREXMIT/LATEREXMIT: the always-on EXP blind-retransmit (handleEXP) is the unified, more-robust recovery for a lost reactive NAK regardless of `peerNakReport` (a `peerNakReport`-gated LATEREXMIT would *regress* tail recovery), so no separate aggressive branch is added — the flag is parsed, stored, and available. Tests `TestConclusionAdvertisesFlags`, `TestPeerNakReportNegotiationUDP`. |
| ~~No OS socket options~~ | **DONE** | `session.ListenUDP(network, laddr, UDPSocketOptions)` builds the `net.PacketConn` with `UDPSendBufSize`/`UDPRecvBufSize`/`IPTTL`/`IPTOS`/`BindToDevice`/`IPv6Only` applied via a `ListenConfig.Control` setsockopt callback (platform-split for `SO_BINDTODEVICE`/js). The façade maps `Config` → `UDPSocketOptions` and feeds the result to Dial/Listen. Test `TestListenUDPSocketOptions` (IP_TTL + SO_RCVBUF read back via getsockopt). |
...
| ~~Periodic NAK suppression / NAKREPORT toggle~~ | **DONE** | `periodicNAK = Congestion!="file" && !DisableNAKReport`: off in file mode, and `DisableNAKReport` (SRTO_NAKREPORT=false) now also suppresses it in live mode — the periodic-NAK timer is neither armed nor fired. The advertised `FlagPeriodicNAK` reflects it. Tests `TestPeriodicNAKModeGating`, `TestNAKReportDisableLiveMode`. |
