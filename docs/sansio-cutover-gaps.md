...
| Gap | Severity | Detail |
|-----|----------|--------|
| **No RTO/EXP timer + no blind retransmit** | **HIGH** | `TimerEXP` is a dead enum value — no `case` in `HandleTimer`, no `retransmitAllInFlight`/FASTREXMIT/LATEREXMIT. The core only retransmits in response to a received NAK. If the **tail of a transfer** (or a NAK itself) is lost with nothing after it, the receiver never detects the gap (it's beyond `maxSeq`), no NAK is sent, and the sender never resends → permanent stall. This is exactly what SRT's EXP timer + blind retransmit exist to prevent. Affects file transfers and any stream that pauses. |
